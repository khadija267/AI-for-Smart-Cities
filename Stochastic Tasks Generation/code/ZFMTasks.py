...
if  name_city!='no' :
   ...
   # wfile = open('punti.txt', 'w')
    ...
    for run_num in range(1,11):
        ...
        '''
        # task txt file
        task_file = open('./Inputs/Mobility/differentradius/50/'+str(run_num)+'/mytask.txt', 'w')
        task_file.write("/ID-Task/  -/Lat/  -/Long/ -/Day/  -/Hour/ -/Minute/   -/Duration/ -/Remaining time/ -/Resources/    -/Coverage/ -/Ligitimacy/ -/on peak hour/ -/grid_number/ -/Task value\n")
        for i in range(0,len(tasks)):
        ...
        # task csv file
        csvfile = open('./Inputs/Mobility/differentradius/50/'+str(run_num)+'/mytask.csv','w')
        with csvfile:
            titles = ['ID','Latitude','Longitude','Day','Hour','Minute','Duration','RemainingTime','Resources','Coverage','Ligitimacy','OnPeakHours','GridNumber', 'Task value']
            writer = csv.DictWriter(csvfile, fieldnames=titles)
            writer.writeheader()
            for i in range(0,len(tasks)):
                # if(tasks[i][3]==0):
                #     continue
                writer.writerow({'ID':tasks[i][0],'Latitude':tasks[i][1],'Longitude':tasks[i][2],'Day':tasks[i][3],'Hour':tasks[i][4],'Minute':tasks[i][5],'Duration':tasks[i][6],'RemainingTime':tasks[i][7],'Resources':tasks[i][8],'Coverage':tasks[i][9],'Ligitimacy':tasks[i][10],'OnPeakHours':tasks[i][11],'GridNumber':tasks[i][12], 'Task value':tasks[i][13]})
